...
fn verify_pool_identity(
    ...
    if trigger.observation_key != snapshot.observation_key {
        return Err("Raydium hydration observation_key changed".to_owned());
    }
...
mod tests {
    ...
    #[test]
    fn hydration_rejects_observation_key_change() -> Result<(), String> {
        let observation = fixture_observation(0, 1_234_567)?;
        let mut payload = fixture_hydration_payload(
            123_500,
            &observation.pool_state.token_0_program,
        ...
        payload["result"]["value"][0] =
            fixture_rpc_account(RAYDIUM_CPMM_PROGRAM_ID, &changed_pool_data);

        let result = parse_hydration_response(&observation, &payload);
        assert!(matches!(result, Err(error) if error.contains("observation_key")));
