...
pub struct PumpSwapExactInputQuote {
 ...
 pub amount_in_consumed_raw: u64, pub amount_in_unspent_raw: u64,
...
fn quote_base_input(
 ...
 .checked_sub(total_fee_raw) .ok_or_else(|| "PumpSwap sell fees exceed raw quote output".to_owned())?;
 if amount_out_raw == 0 {
...
pub fn program_subscribe_request() -> Value { json!({
 "jsonrpc": "2.0",
 "id": 4,
...
pub fn parse_hydration_response(
 ...
 if slot < observation.slot { return Err(format!(
...
pub fn hydrate_normalized_observation(
 ...
 token_a: NormalizedToken { mint: snapshot.pool_state.base_mint.clone(),
...
fn verify_pool_identity(
 ...
 if trigger.index != snapshot.index { return Err("PumpSwap hydration pool index changed".to_owned());
 }
 ...
 *offset = (*offset)
 ...
 data,
 *offset, &format!("{label}[{index}] market_cap_lamports_threshold"),
 )?);
 *offset = (*offset)
 .checked_add(16)
...
fn decode_rpc_account_data(
 ...
 let owner = account
 .get("owner")
 .and_then(Value::as_str) .ok_or_else(|| format!("{label} missing owner"))?;
...
mod tests {
 ...
 #[test] fn rejects_unsorted_dynamic_fee_tiers() -> Result<(), String> {
 let mut data = Vec::new();
 data.extend_from_slice(&FEE_CONFIG_DISCRIMINATOR);
 data.push(PUMPSWAP_FEE_CONFIG_BUMP);
 data.extend_from_slice(&[9u8; 32]);
 ...
 #[test] fn initialized_mint_returns_decimals() -> Result<(), String> {
 let data = initialized_mint(9);
 ...
 fn quote_snapshot( canonical: bool,
 ...
 #[test]
 fn quote_input_buy_matches_official_integer_rounding() -> Result<(), String> { let snapshot = quote_snapshot(true, true)?;
